...
| Existing parsers in `src/core/parsers/` | Parser task | Greenfield |
| Model patterns in `src/core/models/` | Model task | No models exist |
| Utility functions in `src/core/utils/` | Utility task | No utils exist |
...
class RecordType:
    ...
    @classmethod
    def from_raw(cls, data: str, context: ParsingContext) -> Self:
        return cls(
